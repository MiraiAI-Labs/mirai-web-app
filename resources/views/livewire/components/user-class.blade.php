<div class="relative overflow-hidden rounded-lg flex flex-col shadow bg-base-100 p-2 gap-1 {{ $class ?? '' }}">
    ...
    <h1 class="text-4xl font-bold !bg-clip-text text-transparent class-gradient text-center" :class="window.theme == 'dark' ? 'dark' : 'light'">{{ auth()->user()->userStatistic->archetype->name }}</h1>
